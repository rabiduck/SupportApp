...
async function employeeDashboard(db, user, appPage, date, positions, onCall, gatekeepers) {
  ...
  const [pdp, certs, pendingLeave, pendingWfh, scheduled] = await Promise.all([
    ...
    scheduledActionAttention(db, user)
  ]);
