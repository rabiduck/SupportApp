...
async function editEmployeePage(db, id, auth) {
  ...
  const leaveSection = leaveYears.length ? `<div class="form-card section-gap"><h2>Annual Leave Entitlement</h2><p class="muted">Enter the authoritative entitlement for each leave year in days. Adjustments are kept separately for carry-over or exceptional changes.</p><form method="post" action="/employees/${id}/leave-entitlement">${leaveYears.map(y=>{const e=entitlementMap.get(Number(y.id))||{};return `<div class="card section-gap"><strong>${h(y.name)}</strong> <span class="muted">${h(y.start_date)} → ${h(y.end_date)}</span><input type="hidden" name="leave_year_id" value="${y.id}"><label>Entitlement Days<input type="number" name="entitlement_days_${y.id}" min="0" step="0.5" value="${h(e.entitlement_days ?? 0)}"></label><label>Adjustment Days<input type="number" name="adjustment_days_${y.id}" step="0.5" value="${h(e.adjustment_days ?? 0)}"></label><label>Notes<input name="entitlement_notes_${y.id}" maxlength="255" value="${h(e.notes||'')}"></label></div>`}).join('')}<div class="action-bar"><button type="submit">Save Entitlement</button></div></form></div>` : '<div class="notice section-gap"><strong>No leave year configured.</strong><br>A SystemAdmin needs to create a leave year before entitlement can be assigned.</div>';
...
async function saveLeaveEntitlement(request, db, id, auth) {
  ...
  for (const rawYearId of form.getAll('leave_year_id')) {
    ...
    const entitlement=Math.max(0,Number(form.get(`entitlement_days_${yearId}`))||0);
    const adjustment=Number(form.get(`adjustment_days_${yearId}`))||0;
    const notes=String(form.get(`entitlement_notes_${yearId}`)||'').trim()||null;
    await db.prepare(`INSERT INTO employee_leave_entitlements (employee_id,leave_year_id,leave_type_id,entitlement_days,adjustment_days,notes) VALUES (?,?,?,?,?,?)
      ON CONFLICT(employee_id,leave_year_id,leave_type_id) DO UPDATE SET entitlement_days=excluded.entitlement_days,adjustment_days=excluded.adjustment_days,notes=excluded.notes,updated_at=CURRENT_TIMESTAMP`)
...
async function leaveYearsPage(request, db, auth) {
  ...
  const content=`${pageHeader('Leave Years','Define the periods against which annual leave entitlement is recorded.')}<div class="table-card">${table}</div><div class="action-bar section-gap"><button type="button" data-modal-open="create-leave-year">Add Leave Year</button></div><dialog class="app-modal" id="create-leave-year"><div class="modal-head"><h2>Add Leave Year</h2><button type="button" class="modal-close" data-modal-close aria-label="Close">×</button></div><div class="modal-body"><form method="post"><label>Name<input name="name" required placeholder="2026"></label><label>Start Date<input type="date" name="start_date" required></label><label>End Date<input type="date" name="end_date" required></label><div class="action-bar"><button type="submit">Create Leave Year</button><button type="button" class="secondary" data-modal-close>Cancel</button></div></form></div></dialog>`;
