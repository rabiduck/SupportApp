...
async function pdpTeamAssessmentsPage(request,db,user,employeeId=null,cycleId=null){
  ...
  if(employeeId&&cycleId){
    ...
    return appPage('PDP Assessment · '+employee.display_name,cycle.name+' · '+employee.team_name,'<div class="card"><strong>Employee status:</strong> '+h(cycle.participant_status.replace('_',' '))+'</div>'+questionsHtml+scaleHtml+'<form method="post">'+html+'<div class="action-bar section-gap"><button type="submit">Save Manager/TL Ratings</button><a class="button secondary" href="/pdp/team-assessments?cycle='+cycleId+'">Back</a></div></form>',user,'PDP Team Assessments',db);
  }
  const teamParams=teamFilter.length?teamFilter:[],where=teamFilter.length?' AND e.team_id IN ('+teamFilter.map(()=>'?').join(',')+')':'';
  const cycles=await rows(db,`SELECT DISTINCT c.id,c.name,c.start_date,c.due_date FROM pdp_cycle_participants p JOIN pdp_cycles c ON c.id=p.cycle_id JOIN employees e ON e.id=p.employee_id WHERE c.status='published'${where} ORDER BY c.start_date DESC,c.id DESC`,...teamParams);
  const requestedCycleId=Number(new URL(request.url).searchParams.get('cycle')||0),selectedCycleId=cycles.some(c=>Number(c.id)===requestedCycleId)?requestedCycleId:null;
  const params=selectedCycleId?[...teamParams,selectedCycleId]:teamParams,cycleWhere=selectedCycleId?' AND c.id=?':'';
  const items=await rows(db,`SELECT c.id cycle_id,c.name cycle_name,c.due_date,e.id employee_id,e.display_name,t.name team_name,p.status,
    ...
    LEFT JOIN pdp_skill_assessments a ON a.cycle_id=c.id AND a.employee_id=e.id WHERE c.status='published'${where}${cycleWhere}
    GROUP BY c.id,e.id ORDER BY c.due_date IS NULL,c.due_date,t.name,e.display_name`,...params);
  const cycleFilter='<div class="card pdp-cycle-filter"><form method="get"><label>Cycle<select name="cycle"><option value="">All published cycles</option>'+cycles.map(c=>'<option value="'+c.id+'" '+(Number(c.id)===selectedCycleId?'selected':'')+'>'+h(c.name)+'</option>').join('')+'</select></label><button type="submit" class="secondary">Apply Filter</button></form></div>';
...
async function pdpCyclePage(request,db,user,id){
  ...
  if(id&&item.status!=='draft'){
    ...
    const ps=await rows(db,`SELECT e.display_name,t.name team_name,p.status FROM pdp_cycle_participants p JOIN employees e ON e.id=p.employee_id JOIN teams t ON t.id=e.team_id WHERE p.cycle_id=? ORDER BY t.name,e.display_name`,id);
    return appPage('PDP Cycle · '+item.name,'Published cycle details.','<div class="card"><p><strong>Status:</strong> '+h(item.status)+' &nbsp; <strong>Start:</strong> '+h(item.start_date||'—')+' &nbsp; <strong>Due:</strong> '+h(item.due_date||'—')+'</p><h3>Matrices</h3>'+ms.map(m=>'<p><strong>'+h(m.matrix_name)+'</strong> <span class="muted">· '+h(m.team_name)+'</span></p>').join('')+'</div><div class="table-card section-gap"><table><thead><tr><th>Employee</th><th>Team</th><th>Status</th></tr></thead><tbody>'+ps.map(p=>'<tr><td><strong>'+h(p.display_name)+'</strong></td><td>'+h(p.team_name)+'</td><td>'+h(p.status.replace('_',' '))+'</td></tr>').join('')+'</tbody></table></div><div class="action-bar section-gap"><a class="button secondary" href="/pdp/cycles">Back</a></div>',user,'PDP Cycles',db);
  }
...
async function pdpCyclePublish(request,db,user,id){
  ...
  for(const link of links){
    ...
    for(const t of teams){
      ...
      await db.prepare('INSERT OR REPLACE INTO pdp_cycle_matrices(cycle_id,source_matrix_id,team_id,matrix_name,matrix_snapshot) VALUES(?,?,?,?,?)').bind(id,matrix.id,t.team_id,matrix.name,snapshot).run();
    }
...
async function pdpMySkillsPage(request,db,user){
 ...
 const questions=await rows(db,'SELECT q.id,q.question_text,r.response_text FROM pdp_cycle_questions q LEFT JOIN pdp_question_responses r ON r.question_id=q.id AND r.employee_id=? WHERE q.cycle_id=? AND q.is_active=1 ORDER BY q.display_order,q.id',user.id,cycle.id),questionsHtml=questions.length?'<section class="development-questions"><h2>Development Questions</h2>'+questions.map((q,i)=>'<label><strong>'+(i+1)+'. '+h(q.question_text)+'</strong><textarea name="question_'+q.id+'" rows="5" '+(participant.status==='submitted'?'disabled':'')+'>'+h(q.response_text||'')+'</textarea></label>').join('')+'</section>':'';
 ...
 return appPage('My Skills','Rate your current ability and your interest in each skill.',tabs+'<div class="card section-gap"><strong>'+h(cycle.name)+'</strong> · '+status+(cycle.due_date?' <span class="muted">Due '+h(cycle.due_date)+'</span>':'')+'</div>'+scaleHtml+'<form method="post">'+matricesHtml+questionsHtml+actions+'</form>',user,'PDP My Skills',db);
}
