...
async function myLeavePage(request, db, user) {
  ...
  const filterBar=`<form method="get" action="/leave" class="rota-filter"><label>Status<select name="status"><option value="all" ${statusFilter==='all'?'selected':''}>All</option>${['pending','approved','rejected','cancelled'].map(s=>`<option value="${s}" ${statusFilter===s?'selected':''}>${s.replace(/^./,x=>x.toUpperCase())}</option>`).join('')}</select></label><label>Sort<select name="sort"><option value="newest" ${sort==='newest'?'selected':''}>Newest first</option><option value="oldest" ${sort==='oldest'?'selected':''}>Oldest first</option></select></label><label><input type="checkbox" name="past" value="show" ${hidePast?'':'checked'}> Show past leave</label><button type="submit" class="secondary">Apply</button><a class="button secondary" href="/leave">Clear</a></form>`;
