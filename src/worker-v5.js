...
function nav(user, active = '', pdpOutstanding = 0) {
  ...
  const sections = [
    ...
    {name:'Actions', items:[
      ...
      ['All Actions','/actions/all','All Actions'],
      ['Create Action','/actions/new','Create Action'],
      ['Created Actions','/actions/schedules','Created Actions'],
...
function activeForPath(path) {
  ...
  if (path === '/actions/new') return 'Create Action';
  if (path === '/actions/all') return 'All Actions';
  if (path.startsWith('/actions/schedules')) return 'Created Actions';
...
async function pdpMatrixTeamConflicts(db,matrixIds){
  const ids=[...new Set(matrixIds.map(Number).filter(Number.isInteger))];
  if(ids.length<2)return [];
  const qs=ids.map(()=>'?').join(',');
  return rows(db,`SELECT t.id team_id,t.name team_name,GROUP_CONCAT(m.name, ', ') matrix_names,COUNT(DISTINCT mt.matrix_id) matrix_count FROM pdp_matrix_teams mt JOIN teams t ON t.id=mt.team_id JOIN pdp_matrices m ON m.id=mt.matrix_id WHERE mt.matrix_id IN (${qs}) GROUP BY t.id,t.name HAVING COUNT(DISTINCT mt.matrix_id)>1 ORDER BY t.name`,...ids);
}

function pdpMatrixConflictMessage(conflicts){
...
async function pdpTeamAssessmentsPage(request,db,user,employeeId=null,cycleId=null){
  ...
  const teamFilter=user.managedTeamIds||[];
  if(employeeId&&cycleId){
    ...
    if(!employee||!teamFilter.includes(Number(employee.team_id)))return accessPage('Access Denied','You cannot assess this employee.',403);
    const cycle=await row(db,`SELECT c.*,p.status participant_status FROM pdp_cycles c JOIN pdp_cycle_participants p ON p.cycle_id=c.id WHERE c.id=? AND p.employee_id=? AND c.status IN ('published','closed')`,cycleId,employeeId);
    if(!cycle)return accessPage('Assessment Not Found','That employee assessment does not exist.',404);
    if(request.method.toUpperCase()==='POST'){
      if(cycle.status==='closed')return accessPage('Cycle Closed','Closed PDP cycles are read-only.',400);
    ...
    const questions=await rows(db,'SELECT q.id,q.question_text,r.response_text FROM pdp_cycle_questions q LEFT JOIN pdp_question_responses r ON r.cycle_id=q.cycle_id AND r.question_id=q.id AND r.employee_id=? WHERE q.cycle_id=? AND q.is_active=1 ORDER BY q.display_order,q.id',employeeId,cycleId);
    const questionsHtml=questions.length?'<section class="card development-questions management-questions"><h2>Development Questions</h2>'+questions.map((q,i)=>'<div class="question-answer"><h3>'+(i+1)+'. '+h(q.question_text)+'</h3>'+(q.response_text?'<p class="question-answer-text">'+h(q.response_text)+'</p>':'<p class="muted">Not answered yet.</p>')+'</div>').join('')+'</section>':'';
    ...
    const locked=cycle.status==='closed',opts=v=>'<option value="">—</option>'+[1,2,3,4,5].map(n=>'<option value="'+n+'" '+(Number(v)===n?'selected':'')+'>'+n+'</option>').join(''),seen=new Set();
    const html=matrixRows.map(m=>{let snap={categories:[],skills:[]};try{snap=JSON.parse(m.matrix_snapshot)}catch(_){}
      const cats=(snap.categories||[]).map(cat=>{const skills=(snap.skills||[]).filter(s=>Number(s.category_id)===Number(cat.category_id)&&!seen.has(Number(s.skill_id)));skills.forEach(s=>seen.add(Number(s.skill_id)));if(!skills.length)return'';return '<section class="assessment-category"><h3>'+h(cat.name)+'</h3><div class="table-card management-assessment-table"><table><thead><tr><th>Skill</th><th>Employee Ability</th><th>Employee Interest</th><th>Manager/TL Ability</th><th>Manager/TL Interest</th></tr></thead><tbody>'+skills.map(s=>{const r=rm.get(Number(s.skill_id))||{};return '<tr><td><strong>'+h(s.name)+'</strong><div class="muted">'+h(s.description||'')+'</div></td><td>'+h(r.self_ability||'—')+'</td><td>'+h(r.self_interest||'—')+'</td><td><select name="management_ability_'+s.skill_id+'" '+(locked?'disabled':'')+'>'+opts(r.management_ability)+'</select></td><td><select name="management_interest_'+s.skill_id+'" '+(locked?'disabled':'')+'>'+opts(r.management_interest)+'</select></td></tr>'}).join('')+'</tbody></table></div></section>'}).join('');return cats?'<section class="assessment-matrix"><h2>'+h(m.matrix_name)+'</h2>'+cats+'</section>':''}).join('');
    const footer=locked?'<div class="notice section-gap"><strong>Cycle closed.</strong> This assessment is read-only.</div><div class="action-bar section-gap"><a class="button secondary" href="/pdp/team-assessments?cycle='+cycleId+'">Back</a></div>':'<div class="action-bar section-gap"><button type="submit">Save Manager/TL Ratings</button><a class="button secondary" href="/pdp/team-assessments?cycle='+cycleId+'">Back</a></div>';
    return appPage('PDP Assessment · '+employee.display_name,cycle.name+' · '+employee.team_name,'<div class="card"><strong>Cycle:</strong> '+h(cycle.status)+' &nbsp; <strong>Employee status:</strong> '+h(cycle.participant_status.replace('_',' '))+'</div>'+questionsHtml+scaleHtml+'<form method="post">'+html+footer+'</form>',user,'PDP Team Assessments',db);
  }
  const teamParams=teamFilter,where=' AND e.team_id IN ('+(teamFilter.map(()=>'?').join(',')||'NULL')+')';
  const cycles=await rows(db,`SELECT DISTINCT c.id,c.name,c.status,c.start_date,c.due_date FROM pdp_cycle_participants p JOIN pdp_cycles c ON c.id=p.cycle_id JOIN employees e ON e.id=p.employee_id WHERE c.status IN ('published','closed')${where} ORDER BY CASE c.status WHEN 'published' THEN 0 ELSE 1 END,c.start_date DESC,c.id DESC`,...teamParams);
  const requestedCycleId=Number(new URL(request.url).searchParams.get('cycle')||0),selectedCycle=cycles.find(c=>Number(c.id)===requestedCycleId)||null,selectedCycleId=selectedCycle?Number(selectedCycle.id):null;
  const params=selectedCycleId?[...teamParams,selectedCycleId]:teamParams,cycleWhere=selectedCycleId?' AND c.id=?':" AND c.status='published'";
  const items=await rows(db,`SELECT c.id cycle_id,c.name cycle_name,c.due_date,e.id employee_id,e.display_name,t.name team_name,p.status,
    ...
    LEFT JOIN pdp_skill_assessments a ON a.cycle_id=c.id AND a.employee_id=e.id WHERE 1=1${where}${cycleWhere}
    GROUP BY c.id,e.id ORDER BY c.due_date IS NULL,c.due_date,t.name,e.display_name`,...params);
  const cycleFilter='<div class="card pdp-cycle-filter"><form method="get"><label>Cycle<select name="cycle"><option value="">All active cycles</option>'+cycles.map(c=>'<option value="'+c.id+'" '+(Number(c.id)===selectedCycleId?'selected':'')+'>'+h(c.name)+(c.status==='closed'?' (Closed)':'')+'</option>').join('')+'</select></label><button type="submit" class="secondary">Apply Filter</button></form></div>';
  const table=items.length?'<div class="table-card"><table><thead><tr><th>Employee</th><th>Team</th><th>Cycle</th><th>Employee</th><th>Manager/TL</th><th></th></tr></thead><tbody>'+items.map(x=>'<tr><td><strong>'+h(x.display_name)+'</strong></td><td>'+h(x.team_name)+'</td><td>'+h(x.cycle_name)+'</td><td>'+h(x.status.replace('_',' '))+'</td><td>'+x.manager_rated+'/'+x.skill_count+' rated</td><td><a class="button secondary" href="/pdp/team-assessments/'+x.employee_id+'/'+x.cycle_id+'">'+(selectedCycle?.status==='closed'?'View':'Assess')+'</a></td></tr>').join('')+'</tbody></table></div>':'<div class="empty">There are no PDP assessments matching this filter.</div>';
  return appPage('Team PDP Assessments','Review employee self-assessments and record Manager/TL ratings.',cycleFilter+'<div class="section-gap">'+table+'</div>',user,'PDP Team Assessments',db);
}
...
async function pdpCyclePage(request,db,user,id){
  ...
  if(activeMatrices.size&&!selectionConflicts.length){
    const ids=[...activeMatrices],qs=ids.map(()=>'?').join(',');
    eligible=await rows(db,`SELECT DISTINCT e.id,e.display_name,t.name team_name FROM employees e JOIN teams t ON t.id=e.team_id JOIN pdp_matrix_teams mt ON mt.team_id=e.team_id WHERE mt.matrix_id IN (${qs}) AND e.is_active=1 ORDER BY t.name,e.display_name`,...ids);
  }
  if(request.method.toUpperCase()==='POST'){
    ...
    for(const mid of matrixIds)await db.prepare('INSERT INTO pdp_cycle_matrix_links(cycle_id,matrix_id) VALUES(?,?)').bind(id,mid).run();
    await db.prepare('DELETE FROM pdp_cycle_participants WHERE cycle_id=?').bind(id).run();
    for(const eid of employeeIds)await db.prepare("INSERT INTO pdp_cycle_participants(cycle_id,employee_id,status) VALUES(?,?,'not_started')").bind(id,eid).run();
    const questionIds=form.getAll('question_id'),questionTexts=form.getAll('question_text');await db.prepare('DELETE FROM pdp_cycle_questions WHERE cycle_id=?').bind(id).run();for(let qi=0;qi<questionTexts.length;qi++){const qt=String(questionTexts[qi]||'').trim();if(qt)await db.prepare('INSERT INTO pdp_cycle_questions(cycle_id,question_text,display_order) VALUES(?,?,?)').bind(id,qt,qi+1).run();}
    return new Response(null,{status:303,headers:{Location:'/pdp/cycles/'+id+'/edit'}});
  }
  if(id&&item.status!=='draft'){
    const ms=await rows(db,'SELECT cm.matrix_name,t.name team_name FROM pdp_cycle_matrices cm JOIN teams t ON t.id=cm.team_id WHERE cm.cycle_id=? ORDER BY cm.matrix_name,t.name',id);
    const ps=await rows(db,`SELECT e.display_name,t.name team_name,p.status,SUM(CASE WHEN a.management_ability IS NOT NULL THEN 1 ELSE 0 END) manager_rated,COUNT(a.id) skill_count FROM pdp_cycle_participants p JOIN employees e ON e.id=p.employee_id JOIN teams t ON t.id=e.team_id LEFT JOIN pdp_skill_assessments a ON a.cycle_id=p.cycle_id AND a.employee_id=p.employee_id WHERE p.cycle_id=? GROUP BY e.id,t.id,p.status ORDER BY t.name,e.display_name`,id);
    const closeAction=item.status==='published'?'<form method="post" action="/pdp/cycles/'+id+'/close"><button type="submit" class="secondary" onclick="return confirm(\'Close this PDP cycle? Employee and Manager/TL assessments will become read-only.\')">Close Cycle</button></form>':'';
    return appPage('PDP Cycle · '+item.name,(item.status==='closed'?'Closed':'Published')+' cycle details.','<div class="card"><p><strong>Status:</strong> '+h(item.status)+' &nbsp; <strong>Start:</strong> '+h(item.start_date||'—')+' &nbsp; <strong>Due:</strong> '+h(item.due_date||'—')+(item.closed_at?' &nbsp; <strong>Closed:</strong> '+h(String(item.closed_at).slice(0,16).replace('T',' ')):'')+'</p><h3>Matrices</h3>'+ms.map(m=>'<p><strong>'+h(m.matrix_name)+'</strong> <span class="muted">· '+h(m.team_name)+'</span></p>').join('')+'</div><div class="table-card section-gap"><table><thead><tr><th>Employee</th><th>Team</th><th>Self-assessment</th><th>Manager/TL</th></tr></thead><tbody>'+ps.map(p=>'<tr><td><strong>'+h(p.display_name)+'</strong></td><td>'+h(p.team_name)+'</td><td>'+h(p.status.replace('_',' '))+'</td><td>'+p.manager_rated+'/'+p.skill_count+' rated</td></tr>').join('')+'</tbody></table></div><div class="action-bar section-gap"><a class="button secondary" href="/pdp/cycles">Back</a>'+closeAction+'</div>',user,'PDP Cycles',db);
  }
  const matrixChecks=matrices.map(m=>'<label class="check-row"><input type="checkbox" name="matrix" value="'+m.id+'" '+(activeMatrices.has(Number(m.id))?'checked':'')+'> <strong>'+h(m.name)+'</strong> <span class="muted">· '+h(m.teams||'No teams assigned')+'</span></label>').join('');
  const people=selectionConflicts.length?'<div class="empty">Resolve the team overlap before selecting participants.</div>':eligible.length?eligible.map(e=>'<label class="check-row"><input type="checkbox" name="employee_id" value="'+e.id+'" '+((!id||selectedPeople.has(Number(e.id)))?'checked':'')+'> <strong>'+h(e.display_name)+'</strong> <span class="muted">· '+h(e.team_name)+'</span></label>').join(''):'<div class="empty">Select one or more matrices and load eligible employees.</div>';
  const hidden=[...activeMatrices].map(mid=>'<input type="hidden" name="matrix_id" value="'+mid+'">').join('');
  let questions=id?await rows(db,'SELECT id,question_text FROM pdp_cycle_questions WHERE cycle_id=? ORDER BY display_order,id',id):[];if(!id&&!questions.length)questions=[{question_text:'How do you feel you have improved and developed in the last 6 to 12 months / since joining Claritas? Are you satisfied or dissatisfied with what you have achieved and why?'},{question_text:'What have you been doing to aid your development and do you feel you get enough support from Claritas?'},{question_text:'What could you improve to help with your development and also include anything you feel Claritas could do to make this easier for you?'},{question_text:'What do you like and dislike about your role?'}];
  const questionFields='<fieldset><legend>Development Questions</legend><p class="muted">These questions will be presented to each participant as part of this cycle.</p><div id="pdp-question-list">'+questions.map((q,i)=>'<div class="question-edit-row"><input type="hidden" name="question_id" value="'+(q.id||'')+'"><label>Question '+(i+1)+'<textarea name="question_text" rows="3">'+h(q.question_text)+'</textarea></label><button type="button" class="secondary question-remove" onclick="this.closest(\'.question-edit-row\').remove();document.querySelectorAll(\'#pdp-question-list .question-edit-row label\').forEach((x,i)=>{const t=x.childNodes[0];if(t)t.textContent=\'Question \'+(i+1);});">Remove</button></div>').join('')+'</div><button type="button" class="secondary" onclick="const d=document.getElementById(\'pdp-question-list\');const n=d.children.length+1;d.insertAdjacentHTML(\'beforeend\',\'<div class=&quot;question-edit-row&quot;><label>Question \'+n+\'<textarea name=&quot;question_text&quot; rows=&quot;3&quot;></textarea></label><button type=&quot;button&quot; class=&quot;secondary question-remove&quot; onclick=&quot;this.closest(\\\'.question-edit-row\\\').remove()&quot;>Remove</button></div>\')">Add Question</button></fieldset>';
  const conflictNotice=selectionConflicts.length?'<div class="notice"><strong>Team matrix conflict</strong><br>'+h(pdpMatrixConflictMessage(selectionConflicts))+'</div>':'';
  const content=conflictNotice+'<div class="form-card"><form method="get"><fieldset><legend>Matrices</legend>'+matrixChecks+'</fieldset><button type="submit" class="secondary">Load Eligible Employees</button></form><form method="post">'+hidden+'<label>Cycle Name<input name="name" value="'+h(item.name)+'" required maxlength="120"></label><div class="form-grid"><label>Start Date<input type="date" name="start_date" value="'+h(item.start_date||'')+'"></label><label>Due Date<input type="date" name="due_date" value="'+h(item.due_date||'')+'"></label></div><fieldset><legend>Participants</legend>'+people+'</fieldset>'+questionFields+'<div class="action-bar"><button type="submit" '+(selectionConflicts.length?'disabled':'')+'>Save Draft</button><a class="button secondary" href="/pdp/cycles">Cancel</a></div></form></div>'+(id?'<form method="post" action="/pdp/cycles/'+id+'/publish" class="section-gap"><button type="submit" '+(selectionConflicts.length?'disabled':'')+'>Publish Cycle</button> <span class="muted">Publishing freezes every selected matrix and the ability scale for this assessment round.</span></form>':'');
  return appPage(id?'Edit PDP Cycle':'Create PDP Cycle','Select matrices, review eligible staff and prepare the assessment round.',content,user,'PDP Cycles',db);
}
async function pdpCyclePublish(request,db,user,id){
  ...
  if(!links.length)return accessPage('No Matrices','Add at least one matrix before publishing.',400);
  const conflicts=await pdpMatrixTeamConflicts(db,links.map(x=>x.matrix_id));if(conflicts.length)return accessPage('Team Matrix Conflict',pdpMatrixConflictMessage(conflicts),400);
  const scale=await rows(db,'SELECT score,level,explanation FROM pdp_ability_scale ORDER BY score');
  let assessmentCount=0;
  for(const link of links){
    const matrix=await row(db,'SELECT id,name FROM pdp_matrices WHERE id=?',link.matrix_id);if(!matrix)continue;
    const teams=await rows(db,'SELECT team_id FROM pdp_matrix_teams WHERE matrix_id=?',matrix.id);
    const cats=await rows(db,'SELECT mc.category_id,mc.display_order,c.name,c.description FROM pdp_matrix_categories mc JOIN pdp_categories c ON c.id=mc.category_id WHERE mc.matrix_id=? ORDER BY mc.display_order,c.name',matrix.id);
    const skills=await rows(db,'SELECT ms.category_id,ms.skill_id,ms.display_order,s.name,s.description FROM pdp_matrix_skills ms JOIN pdp_skills s ON s.id=ms.skill_id WHERE ms.matrix_id=? ORDER BY ms.category_id,ms.display_order,s.name',matrix.id);
    if(!skills.length)return accessPage('Empty Matrix',matrix.name+' has no skills. Add at least one before publishing.',400);
    const snapshot=JSON.stringify({categories:cats,skills});
    for(const t of teams){
      const relevant=participants.some(p=>Number(p.team_id)===Number(t.team_id));if(!relevant)continue;
      await db.prepare('INSERT INTO pdp_cycle_matrices(cycle_id,source_matrix_id,team_id,matrix_name,matrix_snapshot) VALUES(?,?,?,?,?)').bind(id,matrix.id,t.team_id,matrix.name,snapshot).run();
    }
    for(const p of participants){
      if(!teams.some(t=>Number(t.team_id)===Number(p.team_id)))continue;
      for(const s of skills){await db.prepare('INSERT OR IGNORE INTO pdp_skill_assessments(cycle_id,employee_id,skill_id) VALUES(?,?,?)').bind(id,p.employee_id,s.skill_id).run();assessmentCount++;}
    }
  }
  if(!assessmentCount)return accessPage('No Applicable Assessments','None of the selected participants belong to teams covered by the selected matrices.',400);
  await db.prepare("UPDATE pdp_cycles SET status='published',published_at=CURRENT_TIMESTAMP,ability_scale_snapshot=? WHERE id=?").bind(JSON.stringify(scale),id).run();
...
async function pdpMySkillsPage(request,db,user){
 const cycles=await rows(db,`SELECT c.id,c.name,c.status cycle_status,c.due_date,c.ability_scale_snapshot,p.status FROM pdp_cycle_participants p JOIN pdp_cycles c ON c.id=p.cycle_id WHERE p.employee_id=? AND c.status IN ('published','closed') ORDER BY CASE c.status WHEN 'published' THEN 0 ELSE 1 END,c.due_date IS NULL,c.due_date,c.created_at DESC`,user.id);
 if(!cycles.length)return appPage('My Skills','Your PDP skills assessments.','<div class="empty">You have no PDP assessments.</div>',user,'PDP My Skills',db);
 ...
 if(request.method.toUpperCase()==='POST'){
  if(cycle.cycle_status==='closed')return accessPage('Cycle Closed','Closed PDP cycles are read-only.',400);
  if(participant.status==='submitted')return accessPage('Assessment Submitted','This assessment has already been submitted.',400);
  const form=await request.formData(),action=String(form.get('action')||'save'),questions=await rows(db,'SELECT id FROM pdp_cycle_questions WHERE cycle_id=? AND is_active=1 ORDER BY display_order,id',cycle.id),assessments=await rows(db,'SELECT id,skill_id FROM pdp_skill_assessments WHERE cycle_id=? AND employee_id=?',cycle.id,user.id);let complete=true;
  for(const q of questions){const response=String(form.get('question_'+q.id)||'').trim();if(!response)complete=false;await db.prepare('INSERT INTO pdp_question_responses(cycle_id,employee_id,question_id,response_text,updated_at) VALUES(?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(cycle_id,employee_id,question_id) DO UPDATE SET response_text=excluded.response_text,updated_at=CURRENT_TIMESTAMP').bind(cycle.id,user.id,q.id,response||null).run();}
  ...
  if(action==='submit'){if(!complete)return accessPage('Assessment Incomplete','Please complete every skill rating and development question before submitting.',400);await db.prepare("UPDATE pdp_cycle_participants SET status='submitted',submitted_at=CURRENT_TIMESTAMP WHERE cycle_id=? AND employee_id=?").bind(cycle.id,user.id).run();}
 ...
 const locked=cycle.cycle_status==='closed'||participant.status==='submitted';
 const questions=await rows(db,'SELECT q.id,q.question_text,r.response_text FROM pdp_cycle_questions q LEFT JOIN pdp_question_responses r ON r.question_id=q.id AND r.employee_id=? WHERE q.cycle_id=? AND q.is_active=1 ORDER BY q.display_order,q.id',user.id,cycle.id),questionsHtml=questions.length?'<section class="development-questions"><h2>Development Questions</h2>'+questions.map((q,i)=>'<label><strong>'+(i+1)+'. '+h(q.question_text)+'</strong><textarea name="question_'+q.id+'" rows="5" '+(locked?'disabled':'')+'>'+h(q.response_text||'')+'</textarea></label>').join('')+'</section>':'';
 ...
 const matricesHtml=matrixRows.map(m=>{let snap={categories:[],skills:[]};try{snap=JSON.parse(m.matrix_snapshot)}catch(_){}
  const cats=(snap.categories||[]).map(cat=>{const skills=(snap.skills||[]).filter(s=>Number(s.category_id)===Number(cat.category_id)&&!seen.has(Number(s.skill_id)));skills.forEach(s=>seen.add(Number(s.skill_id)));if(!skills.length)return'';return '<section class="assessment-category"><h3>'+h(cat.name)+'</h3><div class="table-card"><table><thead><tr><th>Skill</th><th>Description</th><th>Ability</th><th>Interest</th></tr></thead><tbody>'+skills.map(s=>{const r=ratingMap.get(Number(s.skill_id))||{};return '<tr><td><strong>'+h(s.name)+'</strong></td><td>'+h(s.description||'—')+'</td><td><select name="ability_'+s.skill_id+'" '+(locked?'disabled':'')+'>'+options(r.self_ability)+'</select></td><td><select name="interest_'+s.skill_id+'" '+(locked?'disabled':'')+'>'+options(r.self_interest)+'</select></td></tr>'}).join('')+'</tbody></table></div></section>'}).join('');return cats?'<section class="assessment-matrix"><h2>'+h(m.matrix_name)+'</h2>'+cats+'</section>':''}).join('');
 const tabs=cycles.length>1?'<div class="action-bar">'+cycles.map(x=>'<a class="button '+(Number(x.id)===Number(cycle.id)?'':'secondary')+'" href="/pdp/my-skills?cycle='+x.id+'">'+h(x.name)+(x.cycle_status==='closed'?' · Closed':'')+'</a>').join('')+'</div>':'';
 const status='<span class="status-badge '+(participant.status==='submitted'?'status-active':'')+'">'+h(participant.status.replace('_',' '))+'</span>',actions=cycle.cycle_status==='closed'?'<div class="notice section-gap"><strong>Cycle closed.</strong> This assessment is retained as read-only history.</div>':participant.status==='submitted'?'<div class="notice section-gap"><strong>Submitted.</strong> Your self-assessment is locked for this cycle.</div>':'<div class="action-bar section-gap"><button class="secondary" name="action" value="save">Save Draft</button><button name="action" value="submit">Submit Assessment</button></div>';
 return appPage('My Skills','Rate your current ability and your interest in each skill.',tabs+'<div class="card section-gap"><strong>'+h(cycle.name)+'</strong> · '+h(cycle.cycle_status)+' · '+status+(cycle.due_date?' <span class="muted">Due '+h(cycle.due_date)+'</span>':'')+'</div>'+scaleHtml+'<form method="post">'+matricesHtml+questionsHtml+actions+'</form>',user,'PDP My Skills',db);
}
...
async function outstandingPdpCount(db,userId){try{return Number((await row(db,"SELECT COUNT(*) c FROM pdp_cycle_participants p JOIN pdp_cycles c ON c.id=p.cycle_id WHERE p.employee_id=? AND c.status='published' AND p.status<>'submitted'",userId))?.c||0);}catch(_){return 0;}}
...
async function leaveRequestReviewPage(request, db, user, id) {
  ...
  const [existingLeave,wfhOverlays,absenceOverlays,sicknessOverlays] = await Promise.all([
    rows(db,"SELECT lr.id,lr.start_date,lr.end_date,lr.start_portion,lr.end_portion,lr.status,e.display_name FROM leave_requests lr JOIN employees e ON e.id=lr.employee_id WHERE e.team_id=? AND e.id<>? AND e.is_active=1 AND lr.status IN ('pending','approved') AND lr.start_date<=? AND lr.end_date>=? ORDER BY lr.start_date,e.display_name",item.team_id,item.employee_id,item.end_date,item.start_date),
    rows(db,"SELECT w.id,w.request_date,w.status,e.display_name FROM wfh_requests w JOIN employees e ON e.id=w.employee_id WHERE e.team_id=? AND e.id<>? AND e.is_active=1 AND w.status IN ('pending','approved') AND w.request_date>=? AND w.request_date<=? ORDER BY w.request_date,e.display_name",item.team_id,item.employee_id,item.start_date,item.end_date),
    rows(db,'SELECT a.id,a.start_date,a.end_date,a.start_portion,a.end_portion,t.name type_name,e.display_name FROM absences a JOIN absence_types t ON t.id=a.absence_type_id JOIN employees e ON e.id=a.employee_id WHERE e.team_id=? AND e.id<>? AND e.is_active=1 AND a.is_active=1 AND a.start_date<=? AND a.end_date>=? ORDER BY a.start_date,e.display_name',item.team_id,item.employee_id,item.end_date,item.start_date),
    rows(db,'SELECT s.id,s.start_date,s.end_date,s.start_portion,s.end_portion,e.display_name FROM sickness s JOIN employees e ON e.id=s.employee_id WHERE e.team_id=? AND e.id<>? AND e.is_active=1 AND s.is_active=1 AND s.start_date<=? AND s.end_date>=? ORDER BY s.start_date,e.display_name',item.team_id,item.employee_id,item.end_date,item.start_date)
  ]);
  const overlayRange=(startDate,endDate,startPortion='FULL',endPortion='FULL')=>{const effectiveStart=startDate===endDate?endPortion:startPortion;return `${h(startDate)} ${h(effectiveStart==='FULL'?'Full Day':effectiveStart)}${endDate!==startDate?` → ${h(endDate)} ${h(endPortion==='FULL'?'Full Day':endPortion)}`:''}`;};
  const overlays=[
    ...existingLeave.map(x=>({date:x.start_date,employee:x.display_name,type:'Annual Leave',period:overlayRange(x.start_date,x.end_date,x.start_portion,x.end_portion),detail:leaveStatus(x.status),href:'/leave-requests/'+x.id})),
    ...wfhOverlays.map(x=>({date:x.request_date,employee:x.display_name,type:'WFH',period:h(x.request_date),detail:leaveStatus(x.status),href:'/wfh-requests/'+x.id})),
    ...absenceOverlays.map(x=>({date:x.start_date,employee:x.display_name,type:'Absence · '+x.type_name,period:overlayRange(x.start_date,x.end_date,x.start_portion,x.end_portion),detail:'Active',href:'/absence/'+x.id+'/edit'})),
    ...sicknessOverlays.map(x=>({date:x.start_date,employee:x.display_name,type:'Sickness',period:overlayRange(x.start_date,x.end_date,x.start_portion,x.end_portion),detail:'Active',href:'/sickness/'+x.id+'/edit'}))
  ].sort((a,b)=>String(a.date).localeCompare(String(b.date))||a.employee.localeCompare(b.employee)||a.type.localeCompare(b.type));
  const overlaysTable=overlays.length?`<table><thead><tr><th>Team Member</th><th>Overlay</th><th>Dates</th><th>Status</th><th></th></tr></thead><tbody>${overlays.map(x=>`<tr><td><strong>${h(x.employee)}</strong></td><td>${h(x.type)}</td><td>${x.period}</td><td>${x.detail}</td><td><a class="button secondary" href="${x.href}">View</a></td></tr>`).join('')}</tbody></table>`:`<div class="empty">No other ${h(item.team_name)} team members have leave, WFH or absence overlays during this request.</div>`;
  const overlaysCard=`<div class="table-card section-gap"><h2>Team Attendance Conflicts</h2><p class="muted">Existing attendance overlays for other ${h(item.team_name)} team members during the requested dates (${h(item.start_date)}${item.end_date!==item.start_date?` → ${h(item.end_date)}`:''}).</p>${overlaysTable}</div>`;
...
export default {
  async fetch(request, env) {
    try {
      ...
      if (/^\/pdp\/cycles\/\d+\/publish$/.test(path) && request.method.toUpperCase()==='POST') return pdpCyclePublish(request,env.DB,user,Number(path.split('/')[3]));
      if (/^\/pdp\/cycles\/\d+\/close$/.test(path) && request.method.toUpperCase()==='POST') return pdpCycleClose(request,env.DB,user,Number(path.split('/')[3]));
