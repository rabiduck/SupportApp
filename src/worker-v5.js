...
async function pdpMatrixPage(request,db,user,id){
  ...
  const matrixSkills=id ? await rows(db,'SELECT ms.id,ms.category_id,ms.skill_id,ms.display_order,s.name,s.description FROM pdp_matrix_skills ms JOIN pdp_skills s ON s.id=ms.skill_id WHERE ms.matrix_id=? ORDER BY ms.category_id,ms.display_order,s.name',id) : [];
  const availableSkills=id ? await rows(db,'SELECT id,name FROM pdp_skills WHERE is_active=1 AND id NOT IN (SELECT skill_id FROM pdp_matrix_skills WHERE matrix_id=?) ORDER BY name',id) : [];
