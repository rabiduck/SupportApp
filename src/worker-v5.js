...
async function leaveRequestReviewPage(request, db, user, id) {
  ...
  const overlays=[
    ...existingLeave.map(x=>({date:x.start_date,type:'Annual Leave',period:overlayRange(x.start_date,x.end_date,x.start_portion,x.end_portion),detail:leaveStatus(x.status),href:'/leave-requests/'+x.id})),
    ...wfhOverlays.map(x=>({date:x.request_date,type:'WFH',period:h(x.request_date),detail:leaveStatus(x.status),href:'/wfh-requests/'+x.id})),
