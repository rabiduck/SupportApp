...
CREATE TABLE IF NOT EXISTS rota_patterns (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL UNIQUE,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS rota_pattern_days (
    ...
    UNIQUE (rota_pattern_id, week_number, day_of_week),
...
INSERT OR IGNORE INTO permissions (code, description) VALUES
    ('MANAGE_USERS', 'Create and maintain portal users'),
    ('MANAGE_TEAMS', 'Create and maintain departments and teams'),
    ('MANAGE_EMPLOYEES', 'Create and maintain employees'),
    ('MANAGE_ROTA', 'Create and maintain rota configuration');

INSERT OR IGNORE INTO roles (name, description, is_system) VALUES
    ('SystemAdmin', 'Full SupportApp administration', 1),
    ('Engineer', 'Standard SupportApp user', 1);

INSERT OR IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permissions p WHERE r.name = 'SystemAdmin';

INSERT OR IGNORE INTO departments (name, description) VALUES
    ('Support', 'Support department');

INSERT OR IGNORE INTO shift_types (name, code, start_time, end_time, is_working_day) VALUES
    ('Off', 'OFF', NULL, NULL, 0),
